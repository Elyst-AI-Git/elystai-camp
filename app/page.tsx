...
import {daysOverdue, runwayWeeks} from '../lib/finance'
...
function Today() {
  const {tasks, metrics, transactions, invoices, settings, sprints, activeSprintId, currentDate, currentPerson, setView} = useCamp()
...
function Lane({person, sprint}: {person: Person; sprint: Sprint}) {
  const {tasks, metrics, invoices, restDays, dailyHours, currentDate, currentPerson, setPreviewPerson, toggleTask, addTask, updateTask, deleteTask, saveDailyHours, toggleRestDay} = useCamp()
  ...
  async function add(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setMessage(''); const form = new FormData(event.currentTarget); const title = String(form.get('title') ?? '').trim(); if (!title) { setMessage('Give this task a title.'); return }; let tier = addTier; if (tier === 'must' && must.length >= 5) { if (!window.confirm('Musts are capped at five. Add this task as Stretch instead?')) return; tier = 'stretch' }; setBusy(true); try { const result = await addTask({id: crypto.randomUUID(), sprintId: sprint.id, owner: person, title, day: currentDate, tier, category: addCategory, status: 'open', carriedCount: 0}); if (result.error) { setMessage(result.error); return }; setAdding(false) } catch { setMessage('Could not save task') } finally { setBusy(false) } }
  ...
  const priorityTasks = [...must.filter((task) => task.status === 'waiting' || task.status === 'blocked'), ...must.filter((task) => task.status !== 'waiting' && task.status !== 'blocked'), ...stretch]
  const activeDay = !isRestDay && (done > 0 || Number(existingHours ?? 0) > 0)
