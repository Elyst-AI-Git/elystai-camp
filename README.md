...
For Supabase-backed local development, copy `.env.example` to `.env.local` and fill in `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, and `SUPABASE_SERVICE_ROLE_KEY`. `.env.local` is git-ignored. Run every file in `supabase/migrations/` in filename order in the Supabase SQL editor (or with the Supabase CLI) before signing in. The latest migrations add the separate personal-money ledger, shared lead pipeline, and workday attendance log. Public sign-up is intentionally not part of Camp.

The mock seed in `lib/mock/data.ts` is intentionally empty for a clean workspace. In no-env preview mode the React Context uses a try/catch-wrapped `camp-clean-v2-*` localStorage cache as a temporary shim; the previous demo cache is not read. Supabase becomes the source of truth as soon as the public URL and anon key are present. Task, finance, calendar, metric, sprint, slip-reason, daily-hours, workday-attendance, rest-day, weekly-goal, personal-money, and lead mutations live in `lib/data/`, check Supabase errors, and show generic UI errors. Personal money is selected by founder and never contributes to business cash totals. Leads are shared between the two accounts and move through New, Contacted, Qualified, Proposal, Won, and Lost. In local preview, click either lane header to inspect Nihal or Shirin; the signed-in build derives the profile from the authenticated email. Today includes sprint switching, task entry, per-person hours, start/finish workday logging, yesterday-task rollover, rest days, two-week consistency, and weekly goals. The service-role client in `lib/supabase/admin.ts` is server-only and is not imported by client code.
