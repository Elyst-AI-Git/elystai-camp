...
import {allCategories, categoryLabel, categoryPalette} from '../lib/category'
...
function TimeGrid({days,blocks,today,onCreate,onEdit,onMove}:{days:string[];blocks:CalendarBlock[];today:string;onCreate:(day:string,start?:number,end?:number)=>void;onEdit:(block:CalendarBlock)=>void;onMove:(block:CalendarBlock,day:string,start:number)=>void}) {
  ...
  function up(event: PointerEvent<HTMLDivElement>,day:string) { if (!selection) return; const start = selection.start; const end = Math.max(start + 30, position(event)); setSelection(null); onCreate(day,start,Math.min(END_HOUR*60,end)) }
...
function MonthGrid({cells,blocks,today,onCreate,onEdit}:{cells:string[];blocks:CalendarBlock[];today:string;onCreate:(day:string,start?:number,end?:number)=>void;onEdit:(block:CalendarBlock)=>void}) {
  ...
  return <section className="calendar-month-grid"><div className="month-weekdays">{['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map((day) => <b key={day}>{day}</b>)}</div><div className="month-cells">{cells.map((day) => {const dayBlocks=blocks.filter((block) => blockDate(block)===day); return <div className={`month-cell ${day.slice(0,7)!==month?'outside':''} ${day===today?'is-today':''}`} key={day} role="button" tabIndex={0} aria-label={`Add event on ${displayDate(day,{weekday:'long',day:'numeric',month:'long'})}`} onClick={() => onCreate(day)} onKeyDown={(event) => {if (event.key === 'Enter' || event.key === ' ') {event.preventDefault(); onCreate(day)}}}><strong>{asDate(day).getDate()}</strong>{dayBlocks.slice(0,4).map((block) => <button type="button" className="month-block" style={blockStyle(block.category)} onClick={(event) => {event.stopPropagation();onEdit(block)}} key={block.id}><i/>{block.title}<small>{localTimePart(block.startAt)}</small></button>)}{dayBlocks.length>4&&<small className="more-blocks">+{dayBlocks.length-4} more</small>}</div>})}</div></section>
}
