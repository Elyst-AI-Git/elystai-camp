...
function goalComplete(goal: WeeklyGoal): boolean {
  return goal.target === undefined ? goal.value > 0 : goal.value >= goal.target
}

export default function WeeklyKpiPanel() {
  const {weeklyGoals, metrics, sprints, activeSprintId, previewPerson, currentDate, addMetric, removeMetric, addWeeklyGoal, updateWeeklyGoal, deleteWeeklyGoal, adjustWeeklyGoal} = useCamp()
  ...
  async function toggleComplete(goal: WeeklyGoal) {
    setBusy(goal.id); setMessage('')
    try {
      const result = await updateWeeklyGoal(goal.id, {value: goalComplete(goal) ? 0 : 1})
      if (result.error) setMessage(result.error)
    } catch {
      setMessage('Could not update weekly goal')
    } finally {
  ...
  return <section className="kpi-panel">
    ...
    <section className="content-person-grid" aria-label="Content published by person">
      {PEOPLE.map((person) => <article className={`content-person ${previewPerson === person ? 'is-current' : ''}`} key={person}>
    ...
    <div className="goal-person-grid">{PEOPLE.map((person) => <article className={`goal-person ${previewPerson === person ? 'is-current' : ''}`} key={person}><header><div><p className="eyebrow">{PERSON_NAME[person]}</p><h3>{previewPerson === person ? 'Your goals' : `${PERSON_NAME[person]}'s goals`}</h3></div><button type="button" className="button quiet goal-add" onClick={() => sprint && setEditor({person})}>+ Add goal</button></header><div className="goal-list">{weeklyGoals.filter((goal) => goal.sprintId === sprint?.id && goal.person === person).map((goal) => { const complete = goalComplete(goal); return <div className={`goal-card ${goal.color} ${complete ? 'goal-complete' : ''}`} key={goal.id}><div className="goal-card-copy"><b>{goal.title}</b>{goal.description && <small>{goal.description}</small>}<em>{goal.target === undefined ? (complete ? 'Completed' : 'No numeric target') : `${goal.value} / ${goal.target}`}</em></div><div className="goal-card-actions">{goal.target === undefined ? <button type="button" className="goal-complete-button" disabled={busy === goal.id} onClick={() => void toggleComplete(goal)}>{complete ? 'Completed' : 'Mark complete'}</button> : <div className="stepper"><button type="button" aria-label={`Decrease ${goal.title}`} disabled={busy === goal.id || goal.value <= 0} onClick={() => void adjust(goal.id, -1)}>−</button><strong>{goal.value}</strong><button type="button" aria-label={`Increase ${goal.title}`} disabled={busy === goal.id} onClick={() => void adjust(goal.id, 1)}>+</button></div>}<button type="button" className="icon-button" aria-label={`Edit ${goal.title}`} disabled={busy === goal.id} onClick={() => setEditor({person, goal})}>✎</button><button type="button" className="icon-button danger" aria-label={`Delete ${goal.title}`} disabled={busy === goal.id} onClick={() => void remove(goal)}>×</button></div></div>})}</div></article>)}</div>
