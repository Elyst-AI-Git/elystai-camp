...
export default function ReviewScreen() {
  ...
  const recentWorkdayDays = useMemo(() => {
    const start = addDays(currentDate, -6)
  ...
  return <section className="review-screen">
    ...
    <section className="review-panel review-weekly-goals"><div className="review-panel-head"><div><p className="eyebrow">Weekly goals</p><h2>Personal goals for this week</h2></div><span className="count-pill">Editable on Today</span></div><div className="review-goal-grid">{PEOPLE.map((person) => <article className="review-weekly-person" key={person}><header><span className={`avatar ${person}`}><img src={`/avatars/${person}-dp.png`} alt=""/></span><h3>{PERSON_NAME[person]}</h3></header>{weeklyGoals.filter((goal) => goal.sprintId === sprint.id && goal.person === person).map((goal) => { const hit = goal.target === undefined ? goal.value > 0 : goal.value >= goal.target; return <div className={`review-weekly-row ${hit ? 'hit' : ''}`} key={goal.id}><span>{hit ? '✓' : '·'}</span><div><b className={hit ? 'goal-title-complete' : ''}>{goal.title}</b>{goal.description && <small>{goal.description}</small>}</div><strong>{goal.target === undefined ? (hit ? 'Done' : '—') : `${goal.value}/${goal.target}`}</strong></div> })}{weeklyGoals.every((goal) => goal.sprintId !== sprint.id || goal.person !== person) && <p className="review-empty-copy">No personal goals added yet.</p>}</article>)}</div></section>
    ...
    <section className="review-panel review-attendance"><div className="review-panel-head"><div><p className="eyebrow">Workday attendance</p><h2>Last 7 days’ start and finish</h2><p className="review-panel-subcopy">The times each person opened and closed Camp over the rolling 7-day window.</p></div><span className="count-pill">Rolling 7 days</span></div><div className="attendance-review-table"><div className="attendance-review-head"><span>Day</span><span>Nihal</span><span>Shirin</span></div>{recentWorkdayDays.map((day) => <div className="attendance-review-row" key={day}><span>{displayDate(day, {weekday: 'short', day: 'numeric', month: 'short'})}</span><AttendanceValue {...(workdayLogs.find((entry) => entry.person === 'nihal' && entry.date === day) ?? {})}/><AttendanceValue {...(workdayLogs.find((entry) => entry.person === 'shirin' && entry.date === day) ?? {})}/></div>)}</div></section>
