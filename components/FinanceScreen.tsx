...
const personalCategories: PersonalFinanceCategory[] = ['Salary','Food (Lunch)','Food (Dinner)','Food (Breakfast)','Food (Tea/Snacks)','Travel','Rent','Mess','Groceries','Cosmetics','Health','Savings','Mobile data/call payment']
...
export default function FinanceScreen() {
  ...
  const [moneyMode,setMoneyMode] = useState<'business'|'personal'>('business')
  ...
  const categories = useMemo(() => Array.from(new Set(['Services', 'Content', 'Website', 'Training', 'Other', 'Mobile data/call payment', ...transactions.map((transaction) => transaction.category)])).sort(), [transactions])
  ...
  const categorySpend = useMemo(() => {
    ...
    return categories.map((name) => [name, totals.get(name) ?? 0] as [string, number]).sort((a,b) => b[1] - a[1])
  }, [transactions, settings.fxRates, categoryMonth, categories])
  ...
  const chartMax = Math.max(1000, Math.ceil(maxMonthly / 1000) * 1000)
  ...
  return <div className="finance-page">
    <section className="finance-intro">
      <div><p className="eyebrow light">Money, with the story intact</p><h2>{moneyMode === 'business' ? 'Cash position' : 'Personal money'}</h2><p>{moneyMode === 'business' ? 'Running balance is anchored in Settings, then updated by every real transaction.' : 'Track each founder’s personal money separately.'}</p></div>
      {moneyMode === 'business' && <button type="button" className="button dark" onClick={() => setShowSettings((open) => !open)}>{showSettings ? 'Close settings' : 'Finance settings'}</button>}
    </section>

    <div className="money-mode-switch" role="tablist" aria-label="Money view"><button type="button" className={moneyMode === 'business' ? 'active' : ''} onClick={() => setMoneyMode('business')}>Business</button><button type="button" className={moneyMode === 'personal' ? 'active' : ''} onClick={() => setMoneyMode('personal')}>Personal</button></div>
    ...
    <article className="finance-panel chart-panel"><p className="eyebrow">Last three months</p><h2>In vs out</h2><div className="legend"><span className="legend-in">Money in</span><span className="legend-out">Money out</span></div><div className="chart-frame"><div className="chart-y-axis">{chartTicks.map((tick) => <span key={tick}>{tick === 0 ? '₹0' : `₹${Math.round(tick / 1000)}k`}</span>)}</div><div className="chart-plot"><div className="chart-grid-lines"><i/><i/><i/></div><div className="month-chart">{monthly.map((month) => <div className="month-column" key={month.key}><div className="month-bars"><i className="bar-in" style={{height:`${Math.max(3,month.inTotal/chartMax*100)}%`}} title={`${money(month.inTotal)} in`}/><i className="bar-out" style={{height:`${Math.max(3,month.outTotal/chartMax*100)}%`}} title={`${money(month.outTotal)} out`}/></div><small>{month.label}</small><em>₹{Math.round(month.inTotal).toLocaleString('en-IN')} · ₹{Math.round(month.outTotal).toLocaleString('en-IN')}</em></div>)}</div></div></div></article>
    ...
    <section className="finance-grid charts"><article className="finance-panel"><div className="category-panel-head"><div><p className="eyebrow">Spend by category</p><h2>Outflows</h2></div><div className="category-month-nav"><button type="button" aria-label="Previous month" onClick={() => shiftCategoryMonth(-1)}>‹</button><strong>{categoryMonthLabel}</strong><button type="button" aria-label="Next month" onClick={() => shiftCategoryMonth(1)}>›</button></div></div>{categorySpend.map(([name,value]) => <div className={`category-bar ${value === 0 ? 'is-zero' : ''}`} key={name}><div><span>{name}</span><b>{money(value)}</b></div><i style={{width:`${value === 0 ? 0 : Math.max(4,value/maxCategory*100)}%`}}/></div>)}</article><article className="finance-panel"><div className="category-panel-head"><div><p className="eyebrow">Revenue by category</p><h2>Inflows</h2></div><div className="category-month-nav"><button type="button" aria-label="Previous month" onClick={() => shiftCategoryMonth(-1)}>‹</button><strong>{categoryMonthLabel}</strong><button type="button" aria-label="Next month" onClick={() => shiftCategoryMonth(1)}>›</button></div></div>{categoryRevenue.map(([name,value]) => <div className={`category-bar revenue ${value === 0 ? 'is-zero' : ''}`} key={name}><div><span>{name}</span><b>{money(value)}</b></div><i style={{width:`${value === 0 ? 0 : Math.max(4,value/maxRevenue*100)}%`}}/></div>)}</article></section>

    </>}

    {moneyMode === 'personal' && <section className="personal-money-panel">
      <div className="personal-money-head">
        <div><p className="eyebrow">Personal money</p><h2>Keep personal spending separate.</h2></div>
